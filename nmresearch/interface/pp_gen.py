...
class PulseProgram:
    ...
    def staberYXX_susp(fc, n_max, xx=False):
        staber = [
            [90, 0, 90, 180, 180, 0],
            [0, 0, 180, 180, 270, 270],
            [0, 180, 180, 90, 90, 0],
        ...
        if xx:
            str_list.append(
                "5m ip28*" + str((shifts[-1][ph_prog_depth - 1] + fc) % 360)
            )
            str_list.append("")
            str_list.append(
                "ph27 = (360) "
    ...
    @staticmethod
    def disorder_fidelity_staber_cal(fc, n_max=25):
        r"""
        ASHDJFHAJSDHFJSDF
        """
        # preliminaries
        ...
        fwd_shifted = [
            [
                (staber[k][idx % ph_prog_depth] + shift + glb_ph) % 360
                for idx, shift in enumerate(shifts[k])
            ]
            for k in range(ph_prog_len)
        ]
    ...
    @staticmethod
    def disorder_fidelity_staber2_cal(fc, n_max=25):
        r"""
        ASHDJFHAJSDHFJSDF
        """
        # preliminaries
    ...
    @staticmethod
    def disorder_fidelity_staberYXX_cal(fc, n_max=25):
        r"""
        ASHDJFHAJSDHFJSDF
        """
        # preliminaries
